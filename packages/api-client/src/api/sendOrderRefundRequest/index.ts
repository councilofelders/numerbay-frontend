import { AxiosInstance } from 'axios';
import { authHeaders } from '../utils';

// Define the types needed for the implementation
type ApiConfig = {
  api: {
    url: string;
  };
  auth: {
    onTokenRead: () => string | null;
  };
};

type Context = {
  client: AxiosInstance;
  config: ApiConfig;
};

export type SendOrderRefundRequestParams = {
  orderId: string | number;
  wallet?: string;
  contact?: string;
  message?: string;
};

export type SendOrderRefundRequestResponse = {
  success: boolean;
  message?: string;
  [key: string]: any;
};

// Keep the original function signature for backward compatibility
export default async function sendOrderRefundRequest(
  context: Context, 
  params: SendOrderRefundRequestParams, 
  customQuery?: any
): Promise<SendOrderRefundRequestResponse> {
  // Create URL object containing full endpoint URL
  const url = new URL(`orders/${params.orderId}/refund-request`, context.config.api.url);
  const token = context.config.auth.onTokenRead();

  const payload = {
    wallet: params.wallet,
    contact: params.contact,
    message: params.message
  };

  try {
    // Use axios to send a POST request
    const { data } = await context.client.post(url.href, payload, authHeaders(token));
    
    // Return data from the API
    return data;
  } catch (error) {
    console.error('Error sending refund request:', error);
    if (error.response) {
      error.response.data.error = error.response.status;
      return error.response.data;
    }
    throw error;
  }
}

