...
// Composables
import Vue from 'vue';
import {facetGetters, useFacet, useCategory} from '@numerbay/numerbay';
...
export default {
  ...
  watch: {
    ...
    facets() {
      ...
      this.selectedSortBy = this.sortBy?.options?.find(o => o.id === (this.$route?.query?.sort || (this.result?.data?.categories[0].tournament ? ((this.result?.data?.categories[0].tournament===8 || this.result?.data?.categories[0].tournament===12)?'0.5corr20v22mmc-down':'fncv42mmc-down') : 'latest')));
    }
  },
  computed: {
    sortBy() {
  ...
  setup() {
    ...
    const facets = computed(() => facetGetters.getGrouped(result.value, ['status', 'ready', 'encryption', 'rank_tc', 'rank_v2_corr20', 'rank_fnc', 'stake', 'return3m', 'num_sales']));
